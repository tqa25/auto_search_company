...
class ExcelWriter:
    ...
    def write_consolidated_report(self, db, output_path: str):
        """
        Writes a consolidated Excel report with two sheets:
        Sheet 1: "Tổng hợp" - 1 row per company.
        Sheet 2: "Chi tiết nguồn" - 1 row per source URL.
        """
        import json
        import os
        ...
        # 1. Setup Sheet 1 (Tổng hợp)
        ws1 = wb.active
        ws1.title = "Tổng hợp"
        
        headers1 = [
            "STT", "Tên công ty", "Mã số thuế", "Địa chỉ", "Số điện thoại", 
            "Email", "Website", "Người đại diện", "Phương thức thu thập", 
            "Điểm tin cậy tối đa", "Trạng thái", "Thời gian cập nhật"
        ]
        ...
        # 2. Setup Sheet 2 (Chi tiết nguồn)
        ws2 = wb.create_sheet(title="Chi tiết nguồn")
        headers2 = [
            "STT", "Tên công ty", "Mã số thuế", "URL nguồn", "Loại nguồn", 
            "Số điện thoại", "Email", "Địa chỉ", "Người đại diện", 
            "Điểm đánh giá URL", "Lý do đánh giá", "Từ khóa tìm kiếm", 
            "Đoạn trích tìm kiếm", "Thời gian thu thập"
        ]
        ...
        for col_idx, h in enumerate(headers2, start=1):
            ...
            cell.alignment = Alignment(horizontal="center", vertical="center")
            ws2.column_dimensions[openpyxl.utils.get_column_letter(col_idx)].width = 20
        ...
        for company in companies:
            ...
            has_gemini = len(gemini_results) > 0
            has_deep = len(deep_scrape_data) > 0
            
            # --- 1. Process Gemini Quick Results ---
            for gr in gemini_results:
                if gr.get('phone'):
                    phones.update(clean_phone(gr['phone']))
                if gr.get('email'):
                ...
                if gr.get('representative') and gr['representative'] != '—':
                    reps.append(gr['representative'].strip())
                if gr.get('confidence') is not None:
                    confidences.append(float(gr['confidence']))
                
                # Write to Sheet 2 (Detailed)
                sources = []
                ...
                for src_url in sources:
                    ...
                    row_data2 = [
                        ...
                        gr.get('email') or "—",
                        gr.get('address') or "—",
                        gr.get('representative') or "—",
                        "—",
                        "—",
                        "—",
                        "—",
                        gr.get('created_at') or "—"
                    ]
                    for col_idx, val in enumerate(row_data2, start=1):
            ...
            for row in deep_scrape_data:
                if row.get('phone'):
                    phones.update(clean_phone(row['phone']))
                if row.get('email'):
                ...
                if row.get('representative') and row['representative'] != '—':
                    reps.append(row['representative'].strip())
                if row.get('confidence_score') is not None:
                    confidences.append(float(row['confidence_score']))
                
                # Write to Sheet 2 (Detailed)
                rel_score = row.get('relevance_score')
                if rel_score is not None:
                    try:
                        rel_score = round(float(rel_score), 1)
                ...
                row_data2 = [
                    ...
                    row.get('source_type') or "Deep Scrape",
                    row.get('phone') or "—",
                    row.get('email') or "—",
                    row.get('address') or "—",
                    row.get('representative') or "—",
                    rel_score if rel_score is not None else "—",
                    row.get('filter_reason') or "—",
                    row.get('search_query') or "—",
                    row.get('search_snippet') or "—",
                ...
                for col_idx, val in enumerate(row_data2, start=1):
                    ...
                    # Style relevance score cell if numeric
            ...
            # --- 3. Build Consolidated Fields ---
            phone_list = sorted(list(phones))
            email_list = sorted(list(emails))
            
            best_phone = ", ".join(phone_list) if phone_list else "—"
            ...
            best_rep = "—"
            if reps:
                valid_reps = [r for r in reps if r and r != '—']
                if valid_reps:
                    best_rep = valid_reps[0]
            
            if has_gemini and has_deep:
                method = "Cả hai"
            elif has_gemini:
                method = "Gemini Quick Search"
            elif has_deep:
                method = "Deep Scrape"
            else:
            ...
            updated_at = company.get('updated_at') or '—'
            ...
            for col_idx, val in enumerate(row_data1, start=1):
